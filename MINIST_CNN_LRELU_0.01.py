...
class Model():
    def __init__(self, batch_size):
        ...
        self.relu1 = LRelu(self.conv1.output_shape, alpha=0.01)
        self.maxpool1 = MaxPooling2D(self.relu1.output_shape)
        self.conv2 = Conv2D(self.maxpool1.output_shape, 24, 3, 1)
        self.relu2 = LRelu(self.conv2.output_shape, alpha=0.01)
...
if __name__ == "__main__":
    ...
    if not os.path.exists(logpath):
        os.mkdir(logpath)
    logdir = logpath + '/LRELU0.01_log.txt'
    print_freq = 50
    val_freq = 200
    DL = DataLoader()
    ...
    with open(logdir, 'w') as logf:
        for epoch in range(20):
            ...
            with open("LRELU_001.pickle", "wb") as fp:   #Pickling
