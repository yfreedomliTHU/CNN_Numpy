...
import time
import pickle
from my_torch import Conv2D, Relu, LRelu, Linear, MaxPooling2D, AvgPooling2D, CrossEntropyLoss, Global_Max_Pooling
...
class Model():
    def __init__(self, batch_size):
        ...
        self.global_max_pool = Global_Max_Pooling(self.maxpool2.output_shape)
        self.fc = Linear(self.global_max_pool.output_shape, 10)
    ...
    def forward(self, x, label):
        ...
        global_pool_out = self.global_max_pool.forward(pool2_out)
    ...
    def SGD(self):
        ...
        self.conv1.SGD(self.relu1.SGD(self.maxpool1.SGD(
                        self.conv2.SGD(self.relu2.SGD(self.maxpool2.SGD(self.global_max_pool.SGD(
...
if __name__ == "__main__":
    ...
    if not os.path.exists(logpath):
        os.mkdir(logpath)
    logdir = logpath + '/GlobalMaxPool_log.txt'
    print_freq = 50
    val_freq = 200
    DL = DataLoader()
    ...
    with open(logdir, 'w') as logf:
        for epoch in range(20):
            ...
            with open("GlobalMaxPool.pickle", "wb") as fp:   #Pickling
            ...
            learning_rate = learning_rate_exponential_decay(1e-3, epoch, 0.1, 10)
